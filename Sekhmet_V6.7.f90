program Sekhmet
  ...
  !
  ! Compilation (fast)
  ! gfortran -o Sekhmet.x Sekhmet_*.f90 -fopenmp -x f95-cpp-input -O3 -ffast-math -march=native -funroll-loops -fno-protect-parens -ftree-vectorize -finline-limit=1500 -finline-functions-called-once -finit-local-zero -flto -ffree-form -fdefault-real-8 -fdefault-double-8 -finit-local-zero -I .
  ! Compilation (debug)
  ! gfortran -o Sekhmet.x Sekhmet_*.f90 -fopenmp -x f95-cpp-input -Wsurprising -Wall -fbounds-check -Wunderflow -fbacktrace -ffpe-trap=zero -finit-local-zero -g -ffree-form -fdefault-real-8 -fdefault-double-8 -finit-local-zero -I .
  ! Execution
  ! ./Sekhmet.x
...
! ################################################################################################################################
  ...
  do while (time < time_end .and. i < nlines)
    ...
    !Menv_base  = Menv_base_array(i-1)  + (sub_step * (Menv_base_array(i)  - Menv_base_array(i-1)))
    ...
    if (Pe <= R_eff_star) then ! If the planet reaches the star's outer layers, it is considered destroyed (for now) and we stop the computations.
    ...
    ! Find the next timestep so as to keep delta_a/a and delta_e/e < eps and larger than 0.1*eps.
...
contains
  ...
  subroutine Compute_Homemade (G, pi, wind_speed, Cd, c_s, k2dt_planet, M_planet, I_planet, M_star, a, dot_M_star, R_planet, e, &
    ...
    if (Special_print == 1) then
      write(Special_print_file, FMT='(I6, ES15.6, ES15.6, ES15.6, ES15.6, ES15.6, ES15.6)') &
        iter, time/sec, dot_a/au, dot_a_drag/au, dot_a_eq_tides_star/au, dot_a_eq_tides_planet/au, &
          (a * (-(2. * dot_M_star / M_star) + dot_M_star / (M_star + M_planet)))/au
    endif
